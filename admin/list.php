...
function studentList(){
  ...
  while (list($sn, $id, $name, $sex, $year, $month, $day, $school, $class, $phone, $note, $place)=$xoopsDB->fetchRow($result)){
    ...
    $main.="<tr align='center'><td>$id</td><td>$name</td><td>$sex</td><td>$birthday</td><td>$school</td><td>$class</td><td>$phone</td><td>$note</td><td>$place</td><td>$modifyLink/$delLink</td></tr>";
...
switch ($op)
{
  ...
  add();
  redirect_header("list.php",3,"新增成功");
  ...
  case "update":
  update($sn);
  redirect_header("list.php",3,"修改成功");
  break;
  ...
  ";
  $main.="<h1>學生名單</h1>";
  $main.="<table id='tfhover' class='tftable'>";
  $main.="<tr><th>編號</th><th>姓名</th><th width='30'>性別</th><th width='80'>出生年月日</th><th>學校</th><th width='30'>班級</th><th>電話</th><th>備註</th><th width='30'>考場</th><th width='40'><a href='list.php?op=addForm'>新增</a></th></tr>";
  $main.=studentList();
  $main.="</table>";
  }
